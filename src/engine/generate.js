 ...
 * @returns
 */
const generate = (blockContainer, topBlockId) => function* substack (thread, context = {}) {
    let block =
    ...
    while (1) {
        ...
        for (const input of Object.values(block.inputs)) {
            if (input.block) {
                args[input.name] = function* evaluate (
                    additionalContext = null
                ) {
                    return yield* generate(blockContainer, input.block)(
                        thread,
        ...
        if (blockFunction) {
            ...
            result =
                    blockFunction instanceof function* () {}.constructor ?
                        yield* blockFunction(args, blockUtility) :
